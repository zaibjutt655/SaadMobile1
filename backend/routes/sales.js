...
// Helper: date range for seller limited views
...
router.get('/', authenticate, async (req, res, next) => {
  try {
    ...
    // Sellers can only view today / yesterday / last7
...
router.get('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'seller') {
      ...
      data.items = data.items.map(i => { delete i.profit; delete i.purchasePrice; return i; });
    }
...
router.post('/', authenticate, [
  ...
  try {
    ...
    // Enrich items with purchase price and name from source records
    const enrichedItems = [];
    let totalAmount = 0;
    ...
    for (const item of req.body.items) {
      ...
      if (item.itemType === 'product' && item.product) {
        ...
        purchasePrice = 0; // Services = 100% profit
      ...
      // Clean up empty ObjectId fields to prevent validation errors
...
// DELETE /api/sales/:id  (soft delete) — OWNER ONLY
